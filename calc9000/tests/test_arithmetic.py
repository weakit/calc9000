from . import s, p, p_str, extra_p


def test_constants():
    assert p_str(f'N[Pi, 15-{extra_p}]') == '3.14159265358979'
    assert p_str(f'N[E, 15-{extra_p}]') == '2.71828182845905'
    assert p_str(f'N[EulerGamma, 15-{extra_p}]') == '0.577215664901533'
    assert p_str(f'N[GoldenRatio, 15-{extra_p}]') == '1.61803398874989'
    assert p_str(f'N[Catalan, 15-{extra_p}]') == '0.915965594177219'
    assert p_str(f'N[Degree, 15-{extra_p}]') == '0.0174532925199433'
...
def test_operations():
    ...
    assert p_str(f'N[100!, 5-{extra_p}]') == '9.3326e+157'
